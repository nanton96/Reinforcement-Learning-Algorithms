...
class HFOEnv(object):
	...
	def startEnv(self):
		if self.numTeammates == 0:
			os.system("./../../../bin/HFO --seed {} --defense-npcs=0 --defense-agents={} --offense-agents=1 --trials 8000 --untouched-time 500 --headless --frames-per-trial 500 --port {} --fullstate &".format(str(self.seed),
				str(self.numOpponents), str(self.port)))
		else :
			os.system("./../../../bin/HFO --seed {} --defense-agents={} --defense-npcs=0 --offense-npcs={} --offense-agents=1 --trials 8000 --headless --untouched-time 500 --frames-per-trial 500 --port {} --fullstate &".format(
	...
	def get_reward(self, status, nextState):
		...
		if status == 1:
			...
			ball_x = nextState[3]
			ball_y = nextState[4]
			reward = 0.05*math.sqrt((ball_x-1)**2+(ball_y-1)**2)
